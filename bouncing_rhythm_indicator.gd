...
func _on_conductor_beat_incremented():
	...
	if conductor.beat_in_bar == 1:
		...
		#tween.tween_property($Sprite2D, "self_modulate", Color(0.5, 0.5, 0.5), 0.21818).from(Color(4, 0.1, 0.4))
		pass
	else:
		# go white/original
		#tween.tween_property($Path2D/PathFollow2D/Sprite2D, "self_modulate", Color(0.5, 0.5, 0.5), 0.5454).from(Color(1.0, 1.0, 1.0))
		#tween.tween_property($Sprite2D, "self_modulate", Color(0.5, 0.5, 0.5), 0.21818).from(Color(0.4, 4.0, 1.3))
		pass
	
func bounce():
	var tween = create_tween()
	tween.set_trans(Tween.TRANS_QUAD)
	tween.tween_property($Sprite2D, "position:y", $Sprite2D.position.y + 200, 60/220.0/2).set_ease(Tween.EASE_IN)
	tween.tween_property($Sprite2D, "position:y", $Sprite2D.position.y, 60/220.0/2).set_ease(Tween.EASE_OUT)
	tween.play()
